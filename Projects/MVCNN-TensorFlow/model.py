...
tf.app.flags.DEFINE_integer('batch_size', g_.BATCH_SIZE, """Number of images to process in a batch.""")
tf.app.flags.DEFINE_float('learning_rate', g_.INIT_LEARNING_RATE, """Initial learning rate.""")
...
def _conv(name, in_, ksize, strides=[1, 1, 1, 1], padding=DEFAULT_PADDING, group=1, reuse=False):
    ...
    def convolve(i, k):
        return tf.nn.conv2d(i, k, strides, padding=padding)
...
def _maxpool(name, in_, ksize, strides, padding=DEFAULT_PADDING):
    pool = tf.nn.max_pool(in_, ksize=ksize, strides=strides, padding=padding, name=name)
...
def load_alexnet_to_mvcnn(sess, caffetf_modelpath):
    ...
    for name in data_dict.keys():
        _load_param(sess, name, data_dict[name])
