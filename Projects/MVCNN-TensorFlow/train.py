...
sys.path.append(parentdir)  # 装载本目录到sys.path
                ...
                if step % g_.VAL_PERIOD == 0:  # and step > 0:
                    ...
                    for val_step, (val_batch_x, val_batch_y) in \
                        ...
                        val_feed_dict = {view_: val_batch_x,
                                         y_: val_batch_y,
                                         keep_prob_: 1.0}
                        val_loss, pred = sess.run(
                            [loss, prediction], feed_dict=val_feed_dict)
                    ...
                    acc = metrics.accuracy_score(
                        val_y[:predictions.size], np.array(predictions))
                    print('%s: step %d, validation loss=%.4f, acc=%f' %
                          (datetime.now(), step, val_loss, acc * 100.))

                    # validation summary
                    val_loss_summ = sess.run(validation_summary, feed_dict={validation_loss: val_loss})
                    val_acc_summ = sess.run(validation_acc_summary, feed_dict={validation_acc: acc})
...
# python train.py --train_dir="./tmp" --caffemodel="./alexnet_imagenet.npy" --learning_rate=0.0001
