...
class Shape:
    ...
    def crop_center(self, size=(227, 227)):
        ...
        :return: 去除白边的图片集
        """
        w, h = self.views.shape[1], self.views.shape[2]  # 256 * 256
        wn, hn = size
        left = w / 2 - wn / 2  # 14
        top = h / 2 - hn / 2  # 14
        right = left + wn  # 241
...
class Dataset:
    ...
    def shuffle(self):
        ...
        :return: 随机数据集
        """
        z = zip(self.listfiles, self.labels)  # 打包
        z = list(z)  # Python3 中需要装换成list才能对其随机
        random.shuffle(z)
        self.listfiles, self.labels = [list(l) for l in zip(*z)]  # 解包
        self.shuffled = True
    ...
    def _batches(self, listfiles, batch_size):
        ...
        for i in range(0, n, batch_size):
            ...
            x = np.zeros((batch_size, self.V, 227, 227, 3))  # batch X
